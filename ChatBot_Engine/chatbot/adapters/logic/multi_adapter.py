...
class MultiLogicAdapter(LogicAdapter):
    """
    MultiLogicAdapter allows chatbot to use multiple logic
    adapters. It has methods that allow chatbot to add an
    ...
    def process(self, statement, tag_processing = None):
        ...
        for adapter in self.adapters:
            if adapter.can_process(statement):
                confidence, output = adapter.process(statement, tag_processing)
